...
int main(void){
	...
	//Sending a message to the receiver at 137.207.82.51:2013
	struct in_addr receiver_sin_address;
	receiver_sin_address.s_addr = inet_addr("137.207.82.51");//nslookup `hostname` at the target machine (here is the same as sender)
	...
	int result = sendto(sender_sd, mail, strlen(mail), 0, (struct sockaddr *)&receiver_sin, sizeof(receiver_sin));
	if (result == -1){
	...
	else
	{
		...
		char buf[10];
		result = recvfrom(sender_sd, buf, 5, 0, 0, 0);
		printf("%s\n", buf);
	}
}
